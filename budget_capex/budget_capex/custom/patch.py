...
from erpnext.accounts.doctype.budget.budget import validate_budget_records,get_item_details
...
def patch_validate_expense_function():
    ...
    try:
        ...
        # 4. Apply the patch
        setattr(target_module, 'validate_expense_against_budget', validate_expense_against_budget)
...
def validate_expense_against_budget(args, expense_amount=0):
	...
	for dimension in default_dimensions + get_accounting_dimensions(as_list=False):
		...
		if (
			...
			and args.account
			and ((frappe.get_cached_value("Account", args.account, "root_type") == "Expense") or (frappe.get_cached_value("Account", args.account, "account_type") == "Fixed Asset"))
		):
